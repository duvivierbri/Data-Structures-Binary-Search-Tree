...
public class BinarySearchTree<K extends Comparable<? super K>, V> implements Iterable<TreeItem<K, V>> {
	...
	public void balance() {
		...
		Vector<TreeItem<Integer, String>> items = new Vector<TreeItem<Integer, String>>();
		@SuppressWarnings("rawtypes")
		TreeItem[] array = new TreeItem[items.capacity()];
		items = driver.getVectorOfTreeItems();
		
		for(int i = 0; i < array.length; i++) {
			array[i] = items.get(i);
		}
		
		this.root = null; //null will later be replaced by a call from balanceTree() :)
	}
...
//~~~~~~~~~~~~~ HOMEWORK METHODS ~~~~~~~~~~~~~~~
	...
	private boolean treeIsBalanced(TreeNode<K, V> node) {
		...
		int rightHeight;
		int difference;
		boolean LTisBalanced;
		boolean RTisBalanced;
		...
		if (node == null) {
			...
			LTisBalanced = treeIsBalanced(node.getLeftChild());
			RTisBalanced = treeIsBalanced(node.getRightChild());
			
			return (difference <= 1) && LTisBalanced && RTisBalanced;
		}
	}
	...
	private TreeNode<K,V> balanceTree(TreeItem<K,V>[] arr, int first, int last) {
		...
		if (treeIsBalanced(this.root) == true) { //if the tree is balanced, just return the balanced tree!
			return this.root;
		}
		...
		while(!(treeIsBalanced(this.root))) {
			int leftChild = (currIndex * 2) + 1;
			int rightChild = (currIndex * 2) + 2;
			
			
			//adding the left child if there is none
			if (arr[leftChild] == null) {
				//finding a node at the end to place as the left child of the current root
				while (arr[getIndex] == null) {
					getIndex--;
				}
				
				arr[leftChild] = arr[getIndex];
				arr[getIndex] = null;
			}
			
			//adding the right child if there is none
			if (arr[rightChild] == null) {
				//finding where we would get the node from to place if left or right child is null
				while (arr[getIndex] == null) {
					getIndex--;
				}
				
				arr[rightChild] = arr[getIndex];
				arr[getIndex] = null;
			}
